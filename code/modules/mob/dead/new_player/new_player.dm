...
/mob/dead/new_player/proc/check_preferences()
	...
	if(client.prefs.job_preferences.len == 0)
		...
		if(has_antags)
			...
			message_admins("[src.ckey] has no jobs enabled, return to lobby if job is unavailable enabled and [client.prefs.be_special.len] antag preferences enabled. This was a 2018 antag rolling technique. The player has been asked to update their job preferences and has been forcefully returned to the lobby.")
