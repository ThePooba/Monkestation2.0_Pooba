...
/datum/component/cleave_attack/proc/perform_sweep(obj/item/item, atom/target, mob/living/user, params)
	...
	var/turf/center_turf = get_turf_in_angle(get_angle(user, target), user_turf, 1)
	...
	new cleave_effect(user.loc, facing_dir)
	...
	user.changeNext_move(CLICK_CD_MELEE * item.swing_speed * swing_speed_mod)
