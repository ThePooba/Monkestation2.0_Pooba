  ...
  duringDrag(e) {
    ...
    if (!center) {
      this.setState((prev) => ({
        ...
        dX: clamp(
          initX + (100 * (e.clientX - prev.pX)) / window.outerWidth,
          0,
          maxX,
        ),
        dY: clamp(
          initY + (100 * (e.clientY - prev.pY)) / window.outerHeight,
          0,
          maxY,
        ),
      }));
      ...
      this.setState({
        dX: clamp(
          (100 * (e.clientX - clientWidth / 2)) / window.outerWidth,
          0,
          maxX,
        ),
        dY: clamp(
          (100 * (e.clientY - clientHeight / 2)) / window.outerHeight,
          0,
          maxY,
        ),
      });
