...
class Stamp extends Component {
  ...
  stamp() {
    ...
    const papers = document.getElementsByClassName(
      'InspectorBooth__Items__paper',
    );
    if (papers.length <= 0) {
      return;
    }
    ...
    return (
      <Fragment>
        <img
          ...
          src={resolveAsset('tray_cover.png')}
        />
        <img
          className={className + down}
          src={resolveAsset(this.icon)}
...
class StampTray extends Component {
  ...
  initRef(ref) {
    ...
    if (this.ref) {
      this.slide_width = -(
        (100 * this.ref.getBoundingClientRect().width) / window.innerWidth -
        this.end_width -
        0.2
      );
    }
  ...
  handleToggle() {
    ...
    setActive((prev) => !prev);
    act('play_sfx', {
      name: active ? 'tray_open' : 'tray_close',
  ...
  createSegs(num) {
    ...
    for (let i = 0; i < num; i++) {
      segs.push(
        <img
          className="InspectorBooth__Tray__segment"
          src={resolveAsset('tray_segment.png')}
          style={`width: 6vw;`}
        />,
      );
    }
  ...
  render() {
    ...
    return (
      <div
        ref={this.initRef}
        className={className}
        style={{ ...styles.slide, ...(active ? styles.out : styles.in) }}
      >
        {stamps.map((stamp) => (
          <span key={stamp.type}>
            {this.createSegs(1)}
        ...
        {this.createSegs(1)}
        <img
          src={resolveAsset('tray_end.png')}
          className={className + '__end'}
          style={styles.tray_end}
...
class Item extends DraggableControl {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    let shrink = document.getElementsByClassName(
      'InspectorBooth__Receptacle--shrink',
    )[0];
  ...
  startDrag(e) {
    ...
    if (this.sfx_startDrag) {
      act('play_sfx', {
        name: this.sfx_startDrag,
        ckey: this.useUser ? config.client?.ckey : null,
      });
    }
  }
  ...
  duringDrag(e) {
    super.duringDrag(e);
    const bins = document.getElementsByClassName(
      'InspectorBooth__Receptacle--shrink',
    );
    let shrink = false;
  ...
  renderItem() {
    return (
      <Fragment>
        ...
        />
        <div
          className={this.className + '-stamps'}
          // eslint-disable-next-line react/no-danger
  ...
  renderItem() {
    return (
      <Fragment>
        ...
        <div
          className={
            'InspectorBooth__Items__idcard-bar ' + this.props.department
          }
        />
        {this.props.color && (
          <img className={this.className + '-sticker ' + this.props.color} />
        )}
        {this.props.picture && (
          <img
            className={this.className + '-picture'}
            src={resolveAsset(this.props.picture)}
          />
        )}
...
const Speaker = (props, context) => {
  ...
  const cooldown = 3000;
  const [timer, setTimer] = useSharedState(
    context,
    'speaker_timer',
    new Date().getTime() - (cooldown + 1),
  );
  return (
    <img
      className="InspectorBooth__Speaker"
      src={resolveAsset('speaker.png')}
      style={style}
      onClick={() => {
...
const Receptacle = (props, context) => {
  ...
  return (
    <div
      className={className + ' ' + className + '--' + props.type}
      style={style}
      data-exec={props.type}
      data-collision={props.collision ?? 'overlap'}
    >
      {props.children}
    </div>
  );
...
export const InspectorBooth = (props, context) => {
  ...
  return (
    <Window width={775} height={500}>
      <div
        className={className}
        style={`background-image: url(${resolveAsset('desk_bg.png')});`}
      >
        {items.papers?.map((item) => (
          <Paperwork
            ...
            y={item.y}
            z={item.z}
            key={item.id + item.x + item.y + item.z + item.text + item.stamps}
          />
        ))}
        {items.idcards?.map((item) => (
          <IDCard
            ...
            y={item.y}
            z={item.z}
            key={item.id + item.x + item.y + item.z}
          />
        ))}
        <Receptacle type={'shrink'} renderAboveItems>
          <Receptacle type={'drop_item'} collision={'inside'}>
            <img
              ...
              src={resolveAsset('window.png')}
            />
          </Receptacle>
          <div className={className + '__Desk'}>
            <img
              className={className + '__Desk__top'}
              src={resolveAsset('desk_top.png')}
            />
            <span className={className + '__Desk__filler'} />
            <img
              className={className + '__Desk__bottom'}
              src={resolveAsset('desk_bottom.png')}
            />
          </div>
          <Receptacle type={'take_item'} />
        </Receptacle>
        <span
          className={className + '__Window__glass'}
          style={`z-index: ${zIndex + 2000};`}
        >
          <img
            className={className + '__Window__glass-icon'}
            src={resolveAsset('window.png')}
          />
        </span>
        <Speaker />
